...
We use the \emph{hashing trick}~\citep{kirsch2006less}: a single MurmurHash3 call produces two 64-bit values $(h_1, h_2)$, from which $k$ hash positions are derived as $j_i = (h_1 + (i+1) \cdot h_2) \bmod m_x, i \in [0,k-1]$. This reduces hash computation from $\mathcal{O}(k)$ to $\mathcal{O}(1)$ while maintaining the pairwise independence required for Bloom filter guarantees.
...
Since overflow events are rare for power-law data (typically $\beta=1-5\%$),
any resulting collisions contribute to the standard false positive rate. To keep a constant amount of hash functions through the cascading layers the Bloom filter size $m_1=\beta   m_0$, e.g., $m_1 = m_0/64$ for $1.5 \%$ overflow.
...
\textbf{Modes.} For the Cascade BF we propose two modes: the standard mode and an optional \emph{minimal increment} (MI) mode, which only increments positions at minimum count, reducing false positive accumulation. Without MI, each insertion increments all $k$ counters; collisions with other items cause overcounting proportional to filter load. With MI, only minimum-value counters are incremented, so overcounting only occurs when all $k$ positions collide with items of equal or higher count---a rare event for high-frequency items.
While standard mode provides 50\% faster insertions when mild overcounting is tolerable, and supports element deletion via cascade decrement, we recommend MI mode for accuracy-critical applications; all evaluation tables use MI mode.

\textbf{Error.} The error of CascadeBFs can be described by the overcount. This is the increase in the item count due to hash collisions. The expected total overcount for $t$ insertions into $n$ unique positions follows from standard Bloom filter analysis~\citep{broder2004network}:
\begin{equation}
\text{err}_{\text{total}} \approx k \cdot \frac{t}{n} \cdot \left(1 - e^{-kn/m}\right)
\end{equation}

% For power-law data, relative error is $\mathcal{O}(1/c)$ for the count $c_p$ of a specific item p ---high-frequency items have lower relative error, which aligns well with typical query patterns that focus on hotspots rather than rare locations.
...
The implementation of CascadeCBF is available as a header-only C++ library(\url(https://anonymous.4open.science/r/cascade-cbf-606B/)). GDELT event data is publicly available from the GDELT Project (\url{https://www.gdeltproject.org}). COVID-19 case data was derived from the Johns Hopkins CSSE repository.
...
% k could be different for both layers?

Standard mode CascadeCBF provides 1.5--2$\times$ higher throughput than SBF-MI due to simpler increment logic. With minimal increment mode enabled, throughput is comparable to SBF-MI but accuracy improves significantly for low-$k$ configurations where false positive collisions would otherwise accumulate. The difference stems from minimal increment mode requiring a preliminary scan to find minimum counters before incrementing, whereas standard mode increments all $k$ positions unconditionally.
