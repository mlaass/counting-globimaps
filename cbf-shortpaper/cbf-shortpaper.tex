...
Multi-layer approaches address the bit-depth tradeoff. Dynamic Count Filters~\cite{aguilar-saborit_dynamic_2006} resize counters dynamically but incur rebuild overhead when many counters overflow simultaneously. Multilayer Compressed CBFs~\cite{ficara_multilayer_2008} use Huffman coding for variable-length counters, trading CPU cycles for memory efficiency. d-Left Counting Bloom Filters~\cite{bonomi2006improved} use d-left hashing with multiple subtables, achieving near-optimal space at the cost of more complex insertion logic.
...
We use the \emph{hashing trick}: a single MurmurHash3 call produces two 64-bit values $(h_1, h_2)$, from which $k$ hash positions are derived as $j_i = (h_1 + (i+1) \cdot h_2) \bmod m_x$. This reduces hash computation from $\mathcal{O}(k)$ to $\mathcal{O}(1)$ while maintaining the pairwise independence required for Bloom filter guarantees. The 128-bit MurmurHash3 output provides excellent distribution for spatial coordinates, which often exhibit spatial autocorrelation that could defeat weaker hash functions.
...
The expected total overcount for $t$ insertions into $n$ unique positions is:
\begin{equation}
...
CountingGloBiMap (MI) & 1536 & 0.22 & 0.11 \\
CascadeCBF (MI) & 2176 & 0.13 & 0.00 \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:multicategory} shows that CascadeCBF achieves perfect category isolation (0\% error) with the fastest insert time, validating multi-dimensional indexing.
...
\textbf{Parameter Selection.} The number of hash functions $k$ controls the accuracy/throughput tradeoff. Our experiments show $k=4$ suffices for typical spatial workloads (0\% error). Layer sizing follows a simple heuristic: Layer~0 handles the common case (low counts), so its size $m_0$ should satisfy the Bloom filter optimality condition $m_0/n \approx 14$ for 1\% false positives. Layer~1 can be significantly smaller---we use $m_1 = m_0/64$ since only $\sim$1\% of elements overflow.
