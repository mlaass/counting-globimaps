...
\FOR{$i = 0$ \TO $k-1$}
  ...
  \FOR{$x = 0$ \TO $\Lambda-1$}
    ...
    \ENDIF
    \STATE $s \gets s + v \cdot 2^{\sigma_x}$ \COMMENT{$\sigma_x$: bit offset for layer $x$}
...
We evaluate on two real geospatial datasets: \textbf{GDELT} (1.9M global news events with coordinates) and \textbf{COVID-19} (1.8M case locations sampled from 182M total). Both datasets exhibit spatial clustering: GDELT concentrates in major news regions (USA, Europe, Middle East) while COVID-19 reflects population density with hotspots in India, Brazil, and the United States.
...
Table~\ref{tab:multicategory} shows that CascadeCBF achieves perfect category isolation (negligible error) with the fastest insert time, validating multi-dimensional indexing.
...
\textbf{Limitations.} CascadeCBF inherits Bloom filter limitations: (1) no native deletion support (though RM-mode Spectral variants enable this), (2) memory must be pre-allocated (unlike CMS which can stream), and (3) hash collisions cause systematic overcount for high-frequency items sharing hash positions. The multi-layer design mitigates but does not eliminate collision effects.
