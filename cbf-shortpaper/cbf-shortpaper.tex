...
Figure~\ref{fig:k_sensitivity} shows that \textbf{CascadeCBF achieves 0\% error at $k \geq 4$}, matching SBF-MI, while CMS requires $k \geq 11$. Standard mode CascadeCBF provides 1.5--2$\times$ higher throughput than SBF-MI; with minimal increment mode, throughput is comparable.
...
CascadeCBF provides efficient cardinality estimation for power-law spatial data through cascading overflow between layers. Experiments show that with minimal increment and independent hashes, CascadeCBF matches the memory efficiency of Spectral Bloom Filters and Count-Min Sketch while supporting counts up to $2^{31}$. The multi-layer design enables exact counts for highly skewed data by cascading to additional layers. Future work includes multiresolution re-randomization and deletion support.
