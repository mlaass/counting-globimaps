...
Geospatial applications generate point data at massive scales---billions of geotagged posts, GPS traces, sensor observations, and mobility records. A key operation is \emph{cardinality estimation}: counting observations per location for hotspot detection, trajectory analysis, and spatial aggregation. Consider urban traffic monitoring where a single intersection may log millions of vehicles per year while suburban roads see only dozens. Or pandemic surveillance where disease cases cluster dramatically in population centers. These applications require counting structures that adapt to wildly varying frequencies across space.
...
Spectral Bloom Filters~\cite{cohen2003spectral} improve accuracy via \emph{minimal increment} (MI)---only incrementing counters at the current minimum count. This reduces overcounting from false positive collisions. Three variants exist: Minimum Selection (MS) queries the minimum counter; Minimal Increment (MI) only increments minimum counters during insertion; and Recurring Minimum (RM) supports deletions by tracking which counters hold the minimum. Count-Min Sketch~\cite{cormode2005improved} provides provable error bounds: estimates exceed true counts by at most $\varepsilon N$ with probability $1-\delta$, requiring $k = \lceil \ln(1/\delta) \rceil$ rows and $w = \lceil e/\varepsilon \rceil$ columns.
...
\begin{table}[ht]
\centering
\caption{Multi-category isolation on GDELT (1.9M events).}
\label{tab:multicategory}
\small
\begin{tabular}{lccc}
\tophline
\textbf{Implementation} & \textbf{Memory} & \textbf{Insert} & \textbf{Error} \\
 & (KB) & (s) & (\%) \\
\middlehline
Spectral BF (MI) & 2048 & 0.15 & 0.00 \\
Count-Min Sketch & 88 & 0.17 & 0.00 \\
CascadeCBF (MI) & 2176 & 0.13 & 0.00 \\
\bottomhline
\end{tabular}
\end{table}
\footnotetext{All experiments use $k=8$ hash functions. CascadeCBF uses 16-bit Layer~0 ($2^{20}$ counters) and 32-bit Layer~1 ($2^{14}$ counters).}

Table~\ref{tab:multicategory} shows that CascadeCBF achieves perfect category isolation (near-zero error) with the fastest insert time, validating multi-dimensional indexing.
...
\tophline
\textbf{Date} & \textbf{Pts} & \textbf{Cum.} & \textbf{SBF} & \textbf{CMS} & \textbf{CCBF} \\
\middlehline
2020-03 & 142 & 142 & 0.0 & 0.0 & 0.0 \\
2020-06 & 7.9K & 8.1K & 0.0 & 0.0 & 0.0 \\
2020-12 & 84K & 92K & 0.0 & 0.0 & 0.0 \\
2021-06 & 183K & 274K & 0.0 & 0.0 & 0.0 \\
2021-12 & 289K & 563K & 0.23 & 0.23 & 0.0 \\
2022-06 & 547K & 1.1M & 0.51 & 0.51 & 0.0 \\
\bottomhline
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{NYC Taxi: urban hotspot accuracy.}
\label{tab:taxi}
\small
\begin{tabular}{lcccc}
\tophline
...
The key advantage of CascadeCBF in urban scenarios is throughput: 1.6$\times$ faster inserts than CMS (0.066s vs.\ 0.108s) due to shared counter arrays requiring fewer memory accesses per operation. For streaming urban analytics where millions of GPS points arrive per second, this throughput difference compounds significantly.
