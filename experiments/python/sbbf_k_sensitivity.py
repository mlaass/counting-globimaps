...
Tests k = 2..8 with constant fill rate by adjusting memory per k.
...
def run_benchmark(
    ...
    # Compute log_num_blocks for target fill rate
...
def main():
    ...
    total_configs = len(MESHES) * len(K_VALUES) * len(INTRA_STRATEGIES) * len(SEED_STRATEGIES)
    current = 0
