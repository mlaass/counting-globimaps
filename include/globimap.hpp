...
public:
  ...
  void put(std::vector<uint64_t> a) { return putp(&a[0], a.size()); }
  void putp(uint64_t *a, size_t len) {
...
#pragma omp critical
  ...
  bool get(std::vector<uint64_t> a) { return getp(&a[0], a.size()); }
  bool getp(uint64_t *a, size_t len) {
